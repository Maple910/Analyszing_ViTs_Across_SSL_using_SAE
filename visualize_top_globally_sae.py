...
def visualize_top_globally_sae(layer_idx, feature_to_visualize):
    """
    データセット全体から、指定されたSAEの特徴に最も強く反応した画像を可視化し、保存します。
    """
    ...
    print("Collecting SAE activations across the entire dataset...")
    ...
    print(f"Visualizing top images for SAE Feature {feature_to_visualize} in Layer {layer_idx}...")
    ...
    for i, global_idx in enumerate(top_indices_global):
        # データセットの__getitem__メソッドを使って画像と変換を一度に取得する
        img_tensor, _ = dataset.__getitem__(global_idx)
    ...
    save_path = os.path.join(save_dir, f"sae_feature_{feature_to_visualize}_layer_{layer_idx}.png")
    plt.savefig(save_path)
    print(f"Visualization saved to {save_path}")
    plt.show()

if __name__ == "__main__":
    visualize_top_globally_sae(LAYER_TO_ANALYZE, FEATURE_TO_VISUALIZE)