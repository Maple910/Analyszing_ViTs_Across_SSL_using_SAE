# compare_attribute_normalaization_oid_mae.py
...
def compare_global_best_attribute(num_images_to_visualize=9):
    ...
    for layer_idx in range(12):
        ...
        # MAE: 正規化スコア
        m_pos = collect_activation_stats_norm(dataloader_attr, layer_idx, vit_model, sae_model, 'MAE')
        m_neg = collect_activation_stats_norm(dataloader_non_attr, layer_idx, vit_model, sae_model, 'MAE')
        std_m = torch.std(torch.cat([m_pos, m_neg], dim=0), dim=0) + 1e-8
        norm_scores_mae = (m_pos.mean(dim=0) - m_neg.mean(dim=0)) / std_m
        best_val_mae, best_idx_mae = torch.max(norm_scores_mae, dim=0)
        global_scores_mae.append((best_val_mae.item(), layer_idx, best_idx_mae.item(), m_pos[:, best_idx_mae.item()].max().item()))
        
        print(f"Layer {layer_idx:2d} | SAE Unit {best_idx_sae.item():5d} (NormScore: {best_val_sae.item():.4f}) | MAE Unit {best_idx_mae.item():5d} (NormScore: {best_val_mae.item():.4f})")
    ...
    all_act_sae, all_act_mae, all_image_paths = [], [], []
    with torch.no_grad():
        for images, paths in tqdm(dataloader_viz, desc="Scanning for Top Images"):
            ...
            h_m = vit_model.blocks[best_mae_layer].mlp.fc1.register_forward_hook(get_activation("m", activations))
            ...
            all_act_mae.append(activations["m"].view(B, -1, D_MODEL*4).abs()[:, 1:, best_mae_idx].max(dim=1)[0].cpu())
            all_image_paths.extend(paths)

    all_act_sae, all_act_mae = torch.cat(all_act_sae), torch.cat(all_act_mae)
    k_viz = min(num_images_to_visualize, len(all_image_paths))
    _, top_idx_sae = torch.topk(all_act_sae, k=k_viz)
    _, top_idx_mae = torch.topk(all_act_mae, k=k_viz)
    ...
    for i, (idx_list, layer, unit, title_p, offset) in enumerate([
        (top_idx_mae, best_mae_layer, best_mae_idx, "MAE", 0),
        ...
        for j in range(k_viz):
            ...
            score = all_act_mae[idx].item() if title_p == "MAE" else all_act_sae[idx].item()
    ...
    plt.suptitle(f"Global Best (Normalized): '{TARGET_ATTRIBUTE}'", fontsize=16)
    plt.tight_layout()
    ...
    with open(txt_path, 'w') as f:
        ...
        for s, m in zip(global_scores_sae, global_scores_mae):
            f.write(f"Layer {s[1]:2d}: SAE Unit {s[2]:5d} (Score: {s[0]:.4f}, Max: {s[3]:.4f}) | MAE Unit {m[2]:5d} (Score: {m[0]:.4f}, Max: {m[3]:.4f})\n")
        f.write("\n" + "="*50 + "\n")
