...
# ファイルパス設定
CONFIG_PATH = "config_oid.py"
SCRIPT_COMPARE = "compare_attribute_normalization_oid_mae.py"
...
def get_best_unit_from_txt(txt_path):
    ...
    # 正規表現で検索: "GLOBAL BEST SAE: Layer X, Unit Y"
...
# --- MAE専用 ヒートマップ生成関数（修正箇所） ---
def generate_mae_heatmap(attribute, layer, unit_id, analysis_path):
    print(f"  -> Generating Consistent MAE Heatmap | {attribute} | L{layer} U{unit_id}...")
    ...
    # 2. SAEロード
    sae_path = cfg_oid.SAE_WEIGHTS_PATH_TEMPLATE.format(layer_idx=layer)
    sae_model = SparseAutoencoder(cfg_oid.D_MODEL, cfg_oid.D_SAE, 0.0).to(DEVICE)
    ...
    for i, img_path in enumerate(target_image_paths[:9]):
        ...
        with torch.no_grad():
            ...
            sae_feats = sae_flat.reshape(B, N, cfg_oid.D_SAE)
    ...
    plt.suptitle(f"MAE Consistent Heatmap | {attribute} | L{layer} Unit {unit_id}", fontsize=14)
    plt.tight_layout()
    
    save_name = f"mae_heatmap_consistent_L{layer}_U{unit_id}_{attribute}.png"
...
# ==========================================
def main():
    print("=== STARTING MAE BATCH EXPERIMENT ===")
    ...
    for attr in TARGET_ATTRIBUTES:
        ...
        # 2. 分析実行 (subprocess)
        print("  -> Running MAE Analysis Script...")
        ret = subprocess.run(f"python {SCRIPT_COMPARE}", shell=True)
        
        if ret.returncode != 0:
            print("  [ERROR] MAE Analysis failed. Skipping viz.")
            continue

        # 3. 結果読み込み & ヒートマップ生成
        # 保存先パスの推定
        base_dir = cfg_oid.NORMALIZE_ANALYSIS_DIR.replace(f"_{cfg_oid.TARGET_ATTRIBUTE}", f"_{attr}")
        ...
        if layer is not None:
            ...
            generate_mae_heatmap(attr, layer, unit, analysis_path)
        else:
