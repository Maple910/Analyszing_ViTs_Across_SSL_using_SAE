...
def set_seed(seed=42):
    ...
    torch.backends.cudnn.deterministic = True # これを入れると完全に固定されます
...
def collect_activation_stats(dataloader, layer_idx, vit_model, sae_model, target_type):
    ...
    max_activations = torch.zeros(dim).to(DEVICE)
    patch_count = 0
...
def compare_global_best_attribute(num_images_to_visualize=9):
    ...
    for layer_idx in range(12):
        ...
        print(f"Layer {layer_idx}: "
              ...
              f"MoCo Unit {best_idx_mae} (Score: {best_score_mae:.6f}, MaxAct: {best_max_act_mae:.6f})")
    ...
    print("\n" + "="*50)
    print(f"✨ GLOBAL BEST SAE (MoCo): Layer {best_sae_layer}, Unit {best_sae_idx}")
    print(f"   Score: {best_sae[0]:.6f}, Max Act (Pos): {best_sae[3]:.6f}")
    print(f"✨ GLOBAL BEST MoCo Neuron: Layer {best_mae_layer}, Unit {best_mae_idx}")
    ...
    msg_mae = f"MoCo Top-{k_viz} Selectivity: {mae_precision:.1f}% ({mae_match}/{mae_total})"
    ...
    for i in range(k_viz):
        ...
        axes[i].imshow(img)
        axes[i].set_title(f"MoCo L{best_mae_layer} N{best_mae_idx}\n{'POS' if is_pos else 'NEG'} | Act: {score:.2f}", 
    ...
    plt.suptitle(f"Global Best (MoCo): '{TARGET_ATTRIBUTE}'", fontsize=16)
    plt.tight_layout()
